...
def download_all_datasets():
    ...
    print("=" * 50)
    nq_train = load_dataset("nq_open", split="train", cache_dir=HF_CACHE_DIR, trust_remote_code=True)
    nq_val = load_dataset("nq_open", split="validation", cache_dir=HF_CACHE_DIR, trust_remote_code=True)
    ...
    print("=" * 50)
    wiki_train = load_dataset("wikitext", "wikitext-103-raw-v1", split="train", cache_dir=HF_CACHE_DIR, trust_remote_code=True)
    wiki_val = load_dataset("wikitext", "wikitext-103-raw-v1", split="validation", cache_dir=HF_CACHE_DIR, trust_remote_code=True)
    ...
    print("=" * 50)
    squad_train = load_dataset("squad", split="train", cache_dir=HF_CACHE_DIR, trust_remote_code=True)
    squad_val = load_dataset("squad", split="validation", cache_dir=HF_CACHE_DIR, trust_remote_code=True)
